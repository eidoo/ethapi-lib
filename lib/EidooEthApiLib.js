...
class EthApiError extends BaseError {}
...
class EidooEthApiLib extends IEidooEthApiLib {
  ...
  async callAsync({ method, endpoint, body }) {
    ...
    try {
      ...
      console.log(err)
      throw new EthApiError(JSON.stringify(err.response.data))
    }
  }
  ...
  async getAccountTxsDetailsAsync(tradingWalletAddress) {
    const method = 'get'
    const endpoint = `/v3/account/${tradingWalletAddress}/txs/details`
    const response = await this.callAsync({ method, endpoint })
    return response
  }
...
module.exports = { EidooEthApiLib, EthApiError }
